...
function PlayState:enter(params)
    ...
    -- generate board tiles based on level
    self.board.level = self.level
    self.board:generateTitles()
...
function PlayState:update(dt)
    ...
    if self.board:hasNoMatches() then
        self.board:reset()
    else
        if self.canInput then
            ...
            if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
                ...
                elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                    ...
                    local tempnewX = newTile.gridX
                    local tempnewY = newTile.gridY
                    ...
                    if self.board:calculateMatches() then 
                        ...
                        newTile.gridX = tempnewX
                        newTile.gridY = tempnewY
                        self.board.tiles[newTile.gridY][newTile.gridX] = newTile
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for k2, tile in pairs(match) do
                self.score = self.score + tile.variety * #match * 50
            end
            local count = 0
            for k2, tile in pairs(match) do
                self.timer = self.timer + 1
